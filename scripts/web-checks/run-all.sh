...
step "1/16 inline JS parses (both live web apps)"
...
step "2/16 PT cannot activate on a production host"
run node scripts/web-checks/pt-production-gate.js

step "3/16 the Maps callback shim (the ride-distance race)"
run node scripts/web-checks/maps-callback-shim.js

step "4/16 requester <-> Gopher messaging crosses the PT bridge"
run node scripts/web-checks/pt-message-relay.js

step "5/16 the age gate: flavour + product form (the context pass)"
run node scripts/web-checks/age-context-pass.js

step "6/16 one lifecycle definition — Request and Connect both delegate"
run node scripts/web-checks/lifecycle-parity.js

step "7/16 a bare deploy.sh survives macOS bash 3.2"
run bash scripts/web-checks/deploy-bare-invocation.sh

step "8/16 web no-show port vs the VENDORED requester rule (never skips)"
run node scripts/web-checks/noshow-parity.js

step "9/16 all THREE no-show implementations agree (mobile / web / prototype)"
run node scripts/web-checks/noshow-three-way.js

step "10/16 is the vendored rule still upstream's? (skips without the clone)"
run node scripts/web-checks/noshow-freshness.js

step "11/16 Request/Connect/prototype parity harness"
...
step "14/16 shared worker helpers are in a scope BOTH call sites can see"
run node scripts/web-checks/helper-scope.js

step "15/16 Go prototype money parsing (a typed \$61.40 is \$61.40, not \$6,140)"
run node scripts/web-checks/go-money-parse.js

step "16/16 Deals home crowns the merchant who PAID, and nobody otherwise"
