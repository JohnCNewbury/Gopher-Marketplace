...
step "1/9  inline JS parses (both live web apps)"
...
step "2/9  PT cannot activate on a production host"
run node scripts/web-checks/pt-production-gate.js

step "3/9  web no-show port vs the VENDORED requester rule (never skips)"
run node scripts/web-checks/noshow-parity.js

step "4/9  all THREE no-show implementations agree (mobile / web / prototype)"
run node scripts/web-checks/noshow-three-way.js

step "5/9  is the vendored rule still upstream's? (skips without the clone)"
run node scripts/web-checks/noshow-freshness.js

step "6/9  Request/Connect/prototype parity harness"
...
step "9/9  Go prototype money parsing (a typed \$61.40 is \$61.40, not \$6,140)"
