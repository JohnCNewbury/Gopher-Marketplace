...
**Build #261 (2026-09-08 23:57Z, commit `313befd8d`, prod env + ad-hoc `IOS_VERSION` 13.9.3): BUILD SUCCESS, TestFlight UPLOAD SUCCESS — 13.9.3 build 861.** Carries !290 (returnURL) and !291 (marks). **Device result (owner, iPhone, 2026-09-09 ~00:25Z): Cash App Pay onboarding SUCCESSFUL on build 861** — the sheet listed Cash App Pay (the returnURL fix works), the Cash App authorisation completed and the method saved. ⛔ **RETRACTED 2026-09-10: this line used to add "(`handleStripeReturnUrl` path exercised on a device for the first time)". That is FALSE — see the AppUrlListener block below.** The onboarding succeeded; the handler never ran, because the component that calls it is mounted by nothing. I inferred the path from the outcome and wrote the inference down as an observation. Owner-reported; screenshot of the Payment Methods list with the Cash App Pay and Apple Pay marks still to be attached below. Remaining unexercised on iOS: the Cash App order-rule pop-up at Review, and a hold on a Cash App / Apple Pay method (the same charge path as cards; §8). **Android (owner, Samsung, 2026-09-09 ~00:40Z, a tonight's-production build): Cash App Pay onboarding SUCCESSFUL** after installing the Cash App app on the phone (the hand-off needs it). **Status of Decision 2 on real devices: Apple Pay ✅ iOS · Google Pay ✅ Android (test env; production pending Google's review) · Cash App Pay ✅ iOS + Android · Link — offered on both, not yet saved by anyone · card scan ✅ iOS, Android after Google approval.**
