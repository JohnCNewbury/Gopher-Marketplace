...
> **Live proof from this Mac (64.99.216.171):** before deploy, `RateLimit-Remaining` started at 28 and skipped numbers mid-window (other users drawing on the shared bucket); after deploy it decrements by exactly one per request. A 45-request parallel burst at 12:00:04 UTC: **30 × 200, 15 × 429**, and the next request after the window admitted. Log attribution confirmed from nginx: in the 11:59:50–12:00:20 UTC window, **16 × 429, all from 64.99.216.171; zero from any other client**, including one that made 29 requests in the same window. **AC5 verified by the owner 8:03 AM ET: inbound "Hello" got the unmonitored-number auto-reply, so the signed Twilio webhook still validates. All six ACs met.**
>
